...
import static com.bstek.urule.builder.KnowledgeBuilder.*;
...
public class RulesRebuilder {
    ...
    private void rebuildCriteria(ResourceLibrary resLibraries, Criteria criteria, Map<String, String> namedMap, boolean forDSL) {
        List<VariableCategory> threadVariableCategoryList = threadVariableLibrary.get(threadLocalId.get());
        VariableCategory threadVariableCategory = null;
        if (threadVariableCategoryList != null && threadVariableCategoryList.size() > 0) {
            threadVariableCategory = threadVariableCategoryList.get(0);
        }
        List<VariableCategory> variableCategories = resLibraries.getVariableCategories();
